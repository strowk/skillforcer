...
fn skill_fresh(
    ...
    if let Some(m) = w.minutes {
        ...
        checks.push((
            elapsed_min <= m,
            format!("skill '{skill}' last loaded {elapsed_min}m ago (window {m}m)"),
        ));
    }
    if let Some(t) = w.turns {
        ...
        checks.push((
            dt <= t,
            format!("skill '{skill}' loaded {dt} turns ago (window {t})"),
        ));
    }
    if let Some(tok) = w.tokens {
        ...
        checks.push((
            dtok <= tok,
            format!("skill '{skill}' loaded {dtok} tokens ago (window {tok})"),
        ));
    }
...
mod tests {
    ...
    fn session_satisfied_by_any_load() {
        ...
        let r = evaluate(
            &req,
            &loads,
            &now("2026-09-12T12:00:00Z", 500, 999999),
            Combine::All,
        );
        assert!(r.satisfied);
    }
    ...
    fn minutes_window_expires() {
        ...
        let r = evaluate(
            &req,
            &loads,
            &now("2026-09-12T10:41:00Z", 5, 200),
            Combine::All,
        );
        assert!(!r.satisfied);
    ...
    fn all_skills_requires_each_fresh() {
        let req = Requires {
            skills: SkillSet::All(vec!["a".into(), "b".into()]),
            windows: Windows {
                session: true,
                ..Default::default()
            },
    ...
    fn turns_window_and_combinator_all() {
        ...
        let r = evaluate(
            &req,
            &loads,
            &now("2026-09-12T10:30:00Z", 41, 200),
            Combine::All,
        );
        assert!(!r.satisfied);
    }
    ...
    fn any_combinator_reason_matches_satisfied() {
        ...
        let r = evaluate(
            &req,
            &loads,
            &now("2026-09-12T10:10:00Z", 41, 200),
            Combine::Any,
        );
        assert!(r.satisfied);
